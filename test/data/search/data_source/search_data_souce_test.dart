...
void main() {
  group("When getGenres() is called", () {
    test("Request successful", () async {
      ...
      final mockResponse = await File("test/data/search/data_source/get_all_search_mock_json_reponse.json").readAsString();
      ...
      final request = verify(() => networkService.execute<SearchResponseDto>(request: captureAny(named: "request"))).captured.first as NetworkRequest<SearchResponseDto>;
      ...
      const expectedAllSearchDto = SearchResponseDto(
        response: SearchListDto(
          docs: [
            SearchDto(idSong: null, tipo: "1", txt: "Lifehouse", url: null, idArtist: 2843, art: "Lifehouse", dns: "lifehouse", img: "3/7/f/2/37f26a39f944f2c63455e1e7aca58c6c-tb.jpg"),
    ...
    test("Request failed", () async {
      ...
      when(() => networkService.execute<SearchResponseDto>(request: captureAny(named: "request"))).thenAnswer((invocation) => SynchronousFuture(Err(ServerError())));
