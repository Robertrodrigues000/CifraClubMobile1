...
void main() {
  test("When call 'getVersionData' and is success should return versionData", () async {
    ...
    final homeDto = _VersionDataDtoMock();
    final versionData = getFakeVersionData();

    final repository = VersionRepositoryImpl(dataSource);

    when(() => dataSource.getVersionData(any(), any())).thenAnswer((_) => SynchronousFuture(Ok(homeDto)));
    when(homeDto.toDomain).thenReturn(versionData);

    final result = await repository.getVersionData(artistDns: "", songDns: "");
