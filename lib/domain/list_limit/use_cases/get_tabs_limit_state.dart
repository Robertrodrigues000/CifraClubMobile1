...
class GetTabsLimitState {
  ...
  GetTabsLimitState(
      this._getTotalSongbookCifras, this._getTabsLimit, this._getProStatusStream, this._listLimitConstants);
  ...
  Stream<ListLimitState> call(int songbookId) {
    return Rx.combineLatestList([_getTotalSongbookCifras(songbookId), _getProStatusStream()]).map((values) {
      ...
      } else if (diff >= 1 && diff <= _listLimitConstants.tabsWarningCountThreshold) {
