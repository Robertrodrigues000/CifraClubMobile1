...
class RefreshAllSongbooks {
  ...
  Future<Result<List<Songbook>, RequestError>> call() {
    return _songbookRepository.getAllSongbooks().then((getAllSongbooks) async {
      if (getAllSongbooks.isSuccess) {
        final response = getAllSongbooks.get()!;
      ...
      return getAllSongbooks.map((value) => value.map((e) => e.songbook).toList());
    });
  }
}
